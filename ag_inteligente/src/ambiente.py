# -*- coding: utf-8 -*-
"""Classe do Ambiente do Agente Inteligente
Versão: 1 
sem os print()"""
...
class Ambiente():
    ...
    # Limpa a interface web do INT
    def apagaInterfaceINT(self):
        comando = "node " + self.caminhoNode + "apagaINT.js"
        self.executaComando(comando)
    ...
    def saidaDQN(self, e, EPISODES, t, epsilon, action, tempoEpiso, reward, LISTAACAO, LISTARECOMP, medias):
        l = []
        l = ["Episode: ", e, "/", EPISODES, "Score: ", t, "epsilon: ", epsilon, "ação: ", action,
             "TempoEpisodio:", tempoEpiso, "Reward:", reward]
        ...
        if os.path.isfile('../python_influxdb/DQNOut.txt'):
            #print("é arquivo")
            saidaDQN = open("../python_influxdb/DQNOut.txt", "a")
    ...
    def escreverProximoEstado(self, valor):
        arquivo = open("proxEstado.txt", "w")
        if valor == 0:
            for i in range(0,3):
                arquivo.write("0 \n")
            arquivo.close()
            arqProxEstado = self.escreverProximoEstado(1)
            return True
        else:
            if valor == 1:
                mediaSalto = int(self.getMediaLatSalto())
                mediaLatFluxo = int(self.getMediaLatSalto())
                mediaFila = int(self.getMediaFila())
                print("medias salva no arquivo proxEtado.txt")
                print("MEdias:", mediaSalto, mediaLatFluxo, mediaFila)
                arquivo.write(str(mediaSalto))
                arquivo.write("\n")
                arquivo.write(str(mediaLatFluxo))
                arquivo.write("\n")
                arquivo.write(str(mediaFila))
                arquivo.write("\n")
                arquivo.close()
                return True
            else:
    ...
    def lerProximoEstado(self):
        vet = []
        arquivo = open("proxEstado.txt", "r")
        print("lendo arq proxEstado.y=txt")
        ...
        print("aqui msalto:", msalto, mfluxo, mfila)

        return msalto, mfluxo, mfila

    def interageAmbiente(self, acao, num):

        valor = acao
        if num == 0:
            #Quando INT é executado pela primeira vez
            if(valor < 0.5):
            ...
            #nas demais vezes apagar a inteface INT primeiro
            if(valor < 0.5):
    ...
    def mediaImprimir(self):
        salto = int(self.getAMediaLatSalto())
        fluxo = int(self.getAMediaLatFluxo())
        fila = int(self.getAMediaFila())
    ...
    def grandeVetor(self):
        ...
        if verifica_Tabelas is not True:
            ...
            print("Cont ZSeros:", self.contador, "Saida:", saida)
            self.contador += 1
            return saida
        else:
            latencia_salto = self.latenciaSalto(var_ambiente)
            latencia_salto_normalizado = self.preparaDadoNormaliza(latencia_salto)
            ...
            grande_vetor = np.concatenate((latencia_salto, fluxo, fila), axis=None)
